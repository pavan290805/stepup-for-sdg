...
const GLOBAL_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .eco-track { display:flex; width:max-content; align-items:center; will-change:transform; }
  .eco-track-l { animation: scrollL var(--dur,22s) linear infinite; }
  .eco-track-r { animation: scrollR var(--dur,22s) linear infinite; }
  .eco-row:hover .eco-track-l,
  .eco-row:hover .eco-track-r { animation-play-state: paused; }

  .logo-pill {
    transition: transform 0.3s ease, box-shadow 0.3s ease, border-color 0.3s ease, opacity 0.3s ease;
    cursor: default;
  }
  .logo-pill:hover { transform: translateY(-4px) scale(1.03); box-shadow: 0 12px 32px rgba(0,0,0,0.22); opacity: 1 !important; }
...
export default function PartnersPage() {
  ...
  return (
    <div className="min-h-fit theme-transition" style={{background:theme.pageBg,color:theme.text,fontFamily:"'Inter',system-ui,sans-serif"}}>
      ...
      {/* -- ECOSYSTEM FLOW -- */}
      <section className="px-6 md:px-10 py-10" style={{borderBottom:`1px solid ${theme.border}`,background:theme.pageBg}}>
        <Reveal>
